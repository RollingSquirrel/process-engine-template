...
1. All tests will be skipped
2. Added maven plugin
3. Configured Wildfly plugin to use credentials admin:admin and connect via http 

You should be ready to deploy on a local wildfly now.

Run `mvn clean wildfly:deploy` to deploy on your local wildfly.
...
### Unit Test
You can run the JUnit test [ProcessTest](src/test/java/demo/template/ProcessTest.java) in your IDE or using:
...
<!-- Tweet
New @Camunda example: Camunda Platform Process Application - A Process Application for [Camunda Platform](http://docs.camunda.org). https://github.com/camunda-consulting/code/tree/master/snippets/process-engine-template
-->
